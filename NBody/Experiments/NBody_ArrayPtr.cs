...
public unsafe struct NBody_ArrayPtr {
  ...
  private static void Init(double* bodies) {
  ...
  private static void Advance(double* bodies, double dt) {
    unchecked {
  ...
  private static double Energy(double* bodies) {
    unchecked {
      double e = 0.0;
  ...
  public static void Main(string[] args) {
    unchecked {
      double* system = stackalloc double[SIZE * 8];
      Init(system);

      Console.Out.WriteLine(Energy(system).ToString("F9"));
      int advancements = args.Length > 0 ? Int32.Parse(args[0]) : 1000;
      while (advancements-- > 0) {
        Advance(system, 0.01d);
      }
      Console.Out.WriteLine(Energy(system).ToString("F9"));
    }
  }
