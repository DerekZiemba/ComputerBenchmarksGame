...
public static unsafe class NBody_StructPtr_Official {
  struct NBody { public double x, y, z, vx, vy, vz, mass; }

  public static void Main(string[] args) {
    unchecked {
      NBody* bodies = stackalloc NBody[5];   // There doesn't seem to be a difference between 
      NBody* last = bodies + 4;              // using stackalloc or fixed pointer.  
      InitBodies(bodies, last);            // stackalloc uses fewer lines though. 

      Console.Out.WriteLine(Energy(bodies, last).ToString("F9"));

      int advancements = args.Length > 0 ? Int32.Parse(args[0]) : 1000;
      while (advancements-- > 0) {
        Advance(bodies, last, 0.01d);
      }
      Console.Out.WriteLine(Energy(bodies, last).ToString("F9"));
    }
  }
  ...
  [MethodImpl(MethodImplOptions.AggressiveInlining)]
  private static void Advance(NBody* bodies, NBody* last, double dt) {
    unchecked {
      for (NBody* bi = bodies; bi < last; ++bi) {
  ...
  [MethodImpl(MethodImplOptions.AggressiveInlining)]
  private static double Energy(NBody* bodies, NBody* last) {
    unchecked {
      double e = 0.0;
      for (NBody* bi = bodies; bi <= last; ++bi) {
        double
